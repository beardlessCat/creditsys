...
//                        switchFreezeAndUnfreezeMenuItem(field, target);
    ...
    function buildTooltip(target){
        ...
        var bindCell = function(body, formatter){
            body.mouseover(function(e){
            	if(e.target.tagName.toLowerCase() == 'td' && $.inArray($(e.target).attr('field'), options.customAttr.tooltip.fields)>-1){
            		var rowIndex = $(e.target).parent().attr('datagrid-row-index');
                    var rowData = $(target).datagrid('getRows')[rowIndex];
            ...
            if(options.customAttr.tooltip.target == 'row'){
                ...
                if(options.customAttr.tooltip.fields && $.isArray(options.customAttr.tooltip.fields)){
                    ...
                    $.each(options.customAttr.tooltip.fields, function(){
                        var field = this;
                        bindCell($(datagrid_body), options.customAttr.tooltip.formatter);
                    });
                }
...
//            console.log('===>doExprotAll');
    ...
    $.extend($.fn.datagrid.methods, {
        ...
        updateRow: function(jq, param){
			return jq.each(function(){
				var row = $(this).datagrid('getRows')[param.index];
				var before = $.extend({}, row);
...
//        return p.length + 1;
    ...
    function simpleDataTransform(options, data){
        ...
        var checked = $.trim(options.checked).length > 0 ? options.checked.split(',') : [];
        ...
        for(var i= 0, len = data.length; i<len; i++){
            if(tmpMap[data[i][parentField]] && data[i][idField] != data[i][parentField]){
                ...
                data[i]['text'] = data[i][textField] || data[i]['text'];
...
(function($){
    ...
    $.fn.treegrid.defaults.loadFilter = function(data, parentId){
        ...
        if(cusOtpions && cusOtpions.parentField){
            ...
            for(var i=0, len = data.rows.length; i<len; i++){
            	if(tmpMap[data.rows[i][parentField]] && tmpMap[data.rows[i][parentField]] != '0' && tmpMap[data.rows[i][idField]] != tmpMap[data.rows[i][parentField]]){
            		if(!tmpMap[data.rows[i][parentField]]['children']){
                        tmpMap[data.rows[i][parentField]]['children'] = [];
                    }

            		data.rows[i][iconField] && (data.rows[i]['iconCls'] = data.rows[i][iconField]);
                    tmpMap[data.rows[i][parentField]]['children'].push(data.rows[i]);
            	}else{
            		data.rows[i][iconField] && (data.rows[i]['iconCls'] = data.rows[i][iconField]);
