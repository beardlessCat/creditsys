...
$(function(){
	...
	$('#querybtn').bind('click', function(){
		var techName = $("#queryName").textbox("getValue");
		initdatagrid(techName);
	})
		  ...
		  $.messager.confirm('提示','您确认删除所选数据?',function(r){
				if(r){
					$.ajax({
						...
						//提交的数据
						data :{technologyId:row.technologyId},
						//成功返回之后调用的函数             
...
//初始化数据格
function initdatagrid(techName){
	$.ajax({
		url:'technology/allTechnology',
		type:'POST',
		dataType:'json',
		data:{
    ...
    $.messager.confirm('提示','您确认删除所选数据?',function(r){
		if(r){
			$.ajax({
				...
				url : "technology/delete",
				//提交的数据
				data :{technologyId:nodes.technologyId},
				//成功返回之后调用的函数             
				success : function(data) {
					if(data.meta.success){
						initdatagrid();
...
<body class="easyui-layout">   
		<div data-options="region:'center'">
				...
				技术名称：     <input id="queryName" class="easyui-textbox"/>
